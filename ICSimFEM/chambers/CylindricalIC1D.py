"""
    Class to define a 1D cylindrical ionization chamber
    with axial symmetry and generate a mesh.
"""
...
class CylindricalIC1D(Chamber):
    ...
    def __init__(self, name: str, Ndw: float, r1: float, r2: float,
        ...
        r1 : float
            Internal radius of the ionization chamber in m.
        r2 : float
            External radius of the ionization chamber in m.
        heigh : float
            Heigh of the chamber. This value is used to calculate the volume
            of the cavity.
        SN : str
        ...
        # modified if needed.
        volume = np.pi * (r2**2 - r1**2) * heigh

        # Call the Chamber constructor.
        super().__init__(name, Ndw, volume, SN, "Cylindrical")

       # Print information about the mesh generation in the output file !
        self._fMesh = "Mesh generated using CylindricalIC1D class"
    ...
    @property
    def heigh(self) -> float:
        return self.__heigh
    
    @heigh.setter
    def heigh(self, value: float) -> None:
        assert isinstance(value, float) and value > 0.0
        self.__heigh = value
    ...
    def _generateMesh(self):
        ...
        # Create two points from the internal and external radius
